...
async def _run_automation_loop_async(force: bool = False):
    ...
    async with session_factory() as db:
        ...
        # ── 7. Plan ONE campaign for the batch using the top-scored lead ─────
        if not selected:
            logger.info("automation_loop: AI selected no leads")
            cfg_row.last_run_at = now_utc
            cfg_row.last_run_summary = {"status": "no_leads_selected", "run_at": now_utc.isoformat()}
            await db.commit()
            return

        top_selection = selected[0]
        top_ctx = lead_context_map.get(top_selection["lead_id"], {})
        top_contact = top_ctx.get("contact")
        top_company = top_ctx.get("company")
        top_angle = top_selection.get("recommended_angle", "save_time")

        plan = await orchestrator.plan_campaign(
            lead_id=top_selection["lead_id"],
            contact={
                "first_name": top_contact.first_name if top_contact else "",
                "last_name": top_contact.last_name if top_contact else "",
                "title": top_contact.title if top_contact else "",
            },
            company={
                "name": top_company.name if top_company else "",
                "industry": top_company.industry if top_company else "",
                "employee_count": top_company.employee_count if top_company else None,
            },
            enrichment_summary=top_ctx.get("enrichment_summary", ""),
            signal_highlights=top_selection.get("signal_highlights", []),
            recommended_angle=top_angle,
        )

        steps = plan.get("steps", [])
        if not steps:
            logger.warning("automation_loop: campaign planner returned no steps — aborting")
            return

        # Build shared sequence (test mode collapses delay_days to 1 minute)
        sequence = [
            {
                "step": s["step"],
                "delay_days": 1 if test_mode_active else s.get("delay_days", 0),
                "angle": s.get("angle", top_angle),
        ...
        campaign = Campaign(
            ...
            tenant_id=tenant_id,
            name=campaign_name,
            description=f"Auto-generated by E2E loop. Angle: {top_angle}. {plan.get('rationale', '')}",
            status="draft",
        ...
        db.add(campaign)
        await db.flush()
        logger.info("automation_loop: created campaign '%s' for %d leads", campaign_name, len(selected))
        ...
        for selection in selected:
            ...
            try:
                ...
                # ── 8c. Generate step-0 email with QA loop (up to 3 attempts) ─
                step_cfg = {**sequence[0], "angle": recommended_angle}
                ...
                send_eta = (
                    ...
                    if test_mode_active
                    else _next_business_send_time(now_utc)
                )
                ...
                send_email.apply_async(args=[str(msg.id)], eta=send_eta)

                # Advance CampaignLead to next step
                ...
                emails_queued += 1
                logger.info(
                    "automation_loop: queued email for %s %s via %s",
                    contact.first_name, contact.last_name, sender.email,
                )
        ...
        # Activate campaign if at least one email was queued
        if emails_queued > 0:
        ...
        summary = {
            ...
            "campaigns_created": 1 if emails_queued > 0 else 0,
